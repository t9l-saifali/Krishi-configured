...
module.exports.createSubCat2 = function(req, res)
{
	upload(req, res, function(err)
	{
		var nameFilter = { 'category_name':req.body.category_name  };
	    var error        = {};               
	    SubCat2.find(nameFilter).exec().then(GetFilter =>{
...
module.exports.updateSubCat2 = function (req, res) {
	upload(req, res, function (err) {
		var banner = '';
		var icon = '';
		var image = '';
...
module.exports.getAllSubCat2 = function(req, res)
{
	var skip = 0;
	var limit = 5;
	var maxCount = 50;
...
module.exports.deleteSubCat2 = function(req, res)
{
...
module.exports.adminFilterSubCat = function(req, res)
{
	...
	if (parentCat_id)
	{
		...
		}).exec(function(err, SubCat2)
		{
			if (err)
			{
				console.log("Error finding SubCat2");
				res.status(400).json(err);
			}
			else
			{
	...
	{
		SubCat2.find().exec(function(err, SubCat2)
		{
			if (err)
			{
				console.log("Error finding SubCat2");
...
module.exports.cat_SubCat_detailList = async (req, res) =>
{
	...
	{
		SubCat2.find().exec(function(err, SubCat2Data)
		{
			if (err)
			{
				console.log("Error finding SubCat2 Data");
			...
			{
				console.log('CategoryData -----=>>>>>', SubCat2Data)
				CategoryData.map((item, index) =>
				{
					SubCat2Data.map((item1, index1) => parseInt(item._id) === parseInt(item1.parentCat_id) ? CategoryData.push('subCat_Data', item1) : item)
				})
				res.status(200).json(
				{
					'message': 'ok',
